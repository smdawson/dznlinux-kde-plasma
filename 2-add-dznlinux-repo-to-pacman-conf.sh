#!/bin/bash
set -e

######################## Variables #######################
#
# b=bold u=underline bl=black r=red g=green
# y=yellow bu=blue m=magenta c=cyan w=white
# endc=end-color enda=end-argument
b='\033[1m'
u='\033[4m'
bl='\E[30m'
r='\E[31m'
g='\E[32m'
y='\E[33m'
bu='\E[34m'
m='\E[35m'
c='\E[36m'
w='\E[37m'
endc='\E[0m'
enda='\033[0m'
version="20260504"

# https://wiki.archlinux.org/index.php/Pacman/Package_signing#Adding_unofficial_keys
echo
echo -e " [${g}✔${endc}]::[${g}${b}Adding DZN-Linux Keys${enda}${endc}]"
echo

echo -e " [${g}✔${endc}]::[${g}${b}Receiving Key for DZN-Linux Packages${enda}${endc}]"
sudo pacman-key --recv-keys BB31837564255477
echo -e " [${g}✔${endc}]::[${g}${b}Verify Fingerprint${enda}${endc}]"
sudo pacman-key --finger BB31837564255477
echo -e " [${g}✔${endc}]::[${g}${b}Local Sign Imported Key${enda}${endc}]"
sudo pacman-key --lsign-key BB31837564255477
echo -e " [${g}✔${endc}]::[${g}${b}Refresh Keys${enda}${endc}]"
sudo pacman-key --refresh-keys

echo
echo -e " [${g}✔${endc}]::[${g}${b}Added DZN-Linux Keys${enda}${endc}]"
echo

echo
echo -e " [${g}✔${endc}]::[${g}${b}Adding DZN-Linux Repos${enda}${endc}]"
echo

echo '
#[dznlinux_repo_testing]
#SigLevel = Required DatabaseOptional
#Server = https://repo.dozzen.me/archlinux/$repo/$arch

[dznlinux_repo]
SigLevel = Required DatabaseOptional
Server = https://repo.dozzen.me/archlinux/$repo/$arch

[dznlinux_repo_3party]
SigLevel = Required DatabaseOptional
Server = https://repo.dozzen.me/archlinux/$repo/$arch
' | sudo tee --append /etc/pacman.conf

sudo pacman -Syyu

echo
echo -e " [${g}✔${endc}]::[${g}${b}DZN-Linux GPG Keys and Repository Added${enda}${endc}]"
echo
echo -e " ${bu}DZN-Linux Installation Scripts Version${b} $version ${enda} ${endc}"
